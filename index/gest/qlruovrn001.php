...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['action']) && $input['action'] === 'delete' && isset($input['custom_id'])) {
        ...
        $sql = "DELETE FROM 网址 WHERE custom_id = '$custom_id'";
    ...
    if (isset($input['action']) && ($input['action'] === 'pause' || $input['action'] === 'resume') && isset($input['custom_id'])) {
        ...
        if ($operation_value === 1) {
            ...
            $sql = "UPDATE 网址 SET 操作=$operation_value, original_name=名称, 名称='' WHERE custom_id='$custom_id'";
        } else {
            // 恢复操作，将 original_name 列的值恢复到名称列
            $sql = "UPDATE 网址 SET 操作=$operation_value, 名称=original_name WHERE custom_id='$custom_id'";
        }
    ...
    // 检查必填字段
    if (isset($input['name'], $input['amount'], $input['count'], $input['custom_id'])) {
        ...
        // 检查是否存在该ID的数据
        $check_sql = "SELECT * FROM 网址 WHERE custom_id = '$custom_id'";
        $check_result = $conn->query($check_sql);

        if ($check_result->num_rows > 0) {
            // 更新数据
            $sql = "UPDATE 网址 SET 名称='$name', 金额='$amount', 网址='$count' WHERE custom_id='$custom_id'";
        } else {
            // 插入数据
            $sql = "INSERT INTO 网址 (名称, 金额, 网址, custom_id) VALUES ('$name', '$amount', '$count', '$custom_id')";
        }
