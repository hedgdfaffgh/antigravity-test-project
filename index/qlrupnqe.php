...
<body>
    ...
    <script>
        ...
        function addRow(rowData = {}) {
            ...
            amountInput.value = rowData['密钥'] || '';
            amountCell.appendChild(amountInput);

            // 商户名
            var merchantNameCell = row.insertCell(cellIndex++);
            var merchantNameInput = document.createElement('input');
            merchantNameInput.type = 'text';
            merchantNameInput.name = 'merchantName';
            merchantNameInput.value = rowData['商户名'] || '';
            merchantNameCell.appendChild(merchantNameInput);

            // 金额
            var moneyCell = row.insertCell(cellIndex++);
            var moneyInput = document.createElement('input');
            moneyInput.type = 'text';
            moneyInput.name = 'money';
            moneyInput.value = rowData['金额'] || '';
            ...
            resetButton.innerText = '生成';
            resetButton.onclick = function() { generateKey(this); }; // 修改为调用generateKey函数
        ...
        function saveRow(button) {
            ...
            var data = {
                ...
                merchantName: row.querySelector('input[name="merchantName"]').value,
                money: row.querySelector('input[name="money"]').value, // 获取金额
        ...
        function generateKey(button) {
            ...
            fetch('/gest/qlrupnqe001.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ custom_id: customId }) // 发送请求以生成密钥
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    ...
                    if (amountInput) {
                        amountInput.value = data.key; // 将生成的密钥显示在密钥输入框内
                    }
                    alert('密钥已生成');
                } else {
                    alert('生成密钥失败: ' + data.message);
                }
            })
            .catch(error => {
                console.error('Error:', error);
