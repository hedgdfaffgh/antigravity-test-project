...
export class GeminiWebClientBrowser {
  ...
  private async chatCompletionsViaDOM(params: {
    ...
    // ── 每次请求前导航到全新对话，避免旧回复残留 ──
    console.log("[Gemini Web Browser] 导航到新对话...");
    ...
    // ── 关键修复：发送前快照当前最后回复，用于去重 ──
    const preSnapshot = await this.extractLastResponseText(page);
    if (preSnapshot) {
      console.log(
        `[Gemini Web Browser] 快照旧回复(${preSnapshot.length}字): ${preSnapshot.slice(0, 60)}...`,
      );
    }
    ...
    for (let elapsed = 0; elapsed < maxWaitMs; elapsed += pollIntervalMs) {
      ...
      const result = await this.extractDomResult(page);

      // 忽略过短内容（<10 字多为按钮残留；正常短回复≥10 字应放行）
      const minLen = 10;
      ...
      // ── 关键：跳过与旧快照相同的回复 ──
      if (result.text && preSnapshot && result.text === preSnapshot) {
        continue;
      }
  ...
  /**
   * 从页面 DOM 提取最后一条模型回复文本（含流式状态检测）
   */
  private async extractDomResult(page: Page): Promise<{ text: string; isStreaming: boolean }> {
    return page.evaluate(() => {
      ...
      const noisePatterns = [
        "Ask Gemini", "问问 Gemini", "Enter a prompt", "输入提示",
        "需要我为你做些什么", "发起新对话", "我的内容", "设置和帮助",
        "制作图片", "创作音乐", "帮我学习", "随便写点什么",
        "给我的一天注入活力", "升级到 Google AI Plus", "正在加载",
        "复制", "分享", "修改", "朗读",
      ];
  ...
  /**
   * 快照：提取当前页面最后回复的纯文本（用于新旧去重）
   */
  private async extractLastResponseText(page: Page): Promise<string | null> {
    const result = await this.extractDomResult(page);
